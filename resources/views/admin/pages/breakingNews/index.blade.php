...
<div class="row g-0">
  <div class="col-lg-12">
    ...
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">All breakingNews </h4>
          ...
          <table id="config-table" class="table display table-striped border no-wrap">
            <thead>
              <tr>
                <th>Status</th>
                <th>Action</th>
                <th>breakingNews title</th>
                <th>breakingNews Category</th>
                <th>Id</th>
                <th>Author</th>
                <th>breakingNews id</th>
                <th>Image</th>
                <th>Date</th>
            ...
            <tbody>
              ...
              <tr>
                <td>
                  @if($item->status == 1)
                  <span class="label label-success">Active</span @else <span class="label label-danger">Inactive</span
                    @endif </td>
                <td>
                  @if($item->status == 1)
                  <a class="btn btn-warning waves-effect btn-circle waves-light"
                    href="{{ route('breakingNews.inactive',$item->id) }}">
                    <i class="fa fa-eye-slash"></i> </a>
                  @else
                  <a class="btn btn-info waves-effect btn-circle waves-light"
                    href="{{ route('breakingNews.active',$item->id) }}">
                    <i class="fa fa-eye"></i> </a> @endif

                  <a class="btn btn-primary waves-effect btn-circle waves-light"
                    href="{{ route('breakingNews.edit',$item->id) }}">
                    <i class="fa fa-edit"></i> </a>
                  <form hidden action="{{ route('breakingNews.destroy',$item->id) }}" id="form{{ $item->id }}">
                    @csrf
                  </form>
